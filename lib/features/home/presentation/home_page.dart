                      ...
                      children: [
                        Builder(
                          builder: (context) {
                            ...
                            if (score >= 0 && score <= 50) { aqiText = l10n.excellent; aqiColor = Colors.greenAccent; }
                            else if (score > 50 && score <= 100) { aqiText = l10n.good; aqiColor = Colors.yellowAccent; }
                            else if (score > 100 && score <= 150) { aqiText = l10n.moderate_air; aqiColor = Colors.orangeAccent; }
                            else if (score > 150) { aqiText = l10n.bad; aqiColor = Colors.redAccent; }
                      ...
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return Column(
                          children: routes.map((route) => Padding(
                            ...
                            child: _RouteCard(title: route.name, distance: '${route.distance} km', time: '45 min', badgeText: 'FÀCIL', badgeColor: Colors.green, teamControl: '75% control'),
                          )).toList(),
                        );
                      }
                    },
...
class _RouteCard extends StatelessWidget {
  final String title, distance, time, badgeText, teamControl;
  final Color badgeColor;

  const _RouteCard({required this.title, required this.distance, required this.time, required this.badgeText, required this.badgeColor, required this.teamControl});
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: badgeColor.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(8)), child: Text(badgeText, style: TextStyle(color: badgeColor, fontSize: 10, fontWeight: FontWeight.bold))),
                  ],
                ),
                const SizedBox(height: 8),
                Row(children: [const Icon(Icons.location_on_outlined, size: 14, color: Colors.grey), const SizedBox(width: 4), Text(distance, style: const TextStyle(fontSize: 12, color: Colors.grey)), const SizedBox(width: 12), const Icon(Icons.schedule, size: 14, color: Colors.grey), const SizedBox(width: 4), Text(time, style: const TextStyle(fontSize: 12, color: Colors.grey))]),
                const SizedBox(height: 8),
